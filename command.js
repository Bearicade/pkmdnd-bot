    ...
    runCommand = function (prefix, client, msg, cmd, args) {
      try {
        ...
        switch (cmd) {
          ...
          "- move [move ID]: Check the move's properties." + "```");
          break;
          ...
          //switch (args[0])  { //display specific 'weak', 'ability' || 'minor/major'

            //case 'weak':  //make sure to refer to #type.id
            //case 'resist':
            let list = "";
            for (let i = 0; i < types[args[0]].weak.length; i++)  {
              if (i != 0) {list += ", ";} //do not begin with comma on first run
              list += types[args[0]].weak[i]; console.log(types[args[0]].weak[i])
            }
            ...
            //default:  //on default, display everything of the type args[0]
          //}
        break;
